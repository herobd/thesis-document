...
\documentclass[ms,letterpaper,equalmargins,oneside,chaptercenter,parttop,lof,lot]{byumsphd} %electronic,twosidetoc,
% Author: Chris Monson
%
...
An individual primitive has two desirable properties: distinctiveness and frequency. Without distinctiveness it will be very difficult to discriminate between different primitive instances. Frequency is valuable as it allows us to get more out of the effort required to discriminate. %and the effort required to learn how to discriminate.
...
%While there are a plethora of errors we corrected in the ground truth, the ground truth we use differs significantly from that which is officially released, though it is based on it. 
...
\begin{subfigure}{.99\textwidth}
  ...
  %\label{fig:benthamBiSpot}
\end{subfigure}
    \caption{These show QbS mAP for 18 n-grams on the Bentham dataset for varying sliding window sizes (in pixels). Figure \ref{fig:exampleWindows} shows examples of some windows sizes.
    }
...
%Note that the suffixes achieving 100\% AP occur only once or twice. 
...
%However, there is not always a clean division. 
...
The individual n-specific character vectors are summed. % and each letter normalized according to how many n-specific vectors contributed to it.
...
\begin{algorithm}
\caption{Character probability vector creation}\label{alg_cpv}
\SetKwData{PHOCs}{PHOCs}\SetKwData{Char}{character}\SetKwData{Offset}{offset}\SetKwData{Scores}{possibleScores}\SetKwData{CharacterSums}{characterSums}\SetKwData{CharacterCounts}{characterCounts}\SetKwData{Thresh}{thresholdScore}\SetKwData{FinalCharacterSums}{CPV}\SetKwData{Contrib}{numContributing}\SetKwData{MinValue}{minValue}\SetKwData{Ngram}{n-gram}\SetKwData{Ones}{1s}
\SetKwFunction{Spot}{DenseSpot}\SetKwFunction{OffsetF}{OffsetVector}\SetKwFunction{GetOffset}{GetOffset}\SetKwFunction{SoftMax}{SoftMax}\SetKwFunction{CreateBatch}{CreateBatch}
\SetKwInOut{Input}{input}\SetKwInOut{Output}{output}
 \Input{word image}
 ...
 \ForEach{$n$ in \{$1,2,3$\}}{
   \ForEach{\Ngram of length $n$}{
     \PHOCs $:= $ \Spot{\Ngram}\tcc*[r]{PHOC vector for each window location along word image}
   %}   
   \ForEach{position $l$ in \Ngram}{
     \Char $:=$ \Ngram$[l]$\;
     \Offset $:=$ \GetOffset($l$,$n$,\Ngram)\tcc*[r]{Based on estimated character width and window width}
     \CharacterSums$[n][$\Char$]$ $:=$ \CharacterSums$[n][$\Char$]$ + \OffsetF{\Offset,\PHOCs}\;
     \CharacterCounts$[n][$\Char$]$ := \CharacterCounts$[n][$\Char$]$+\OffsetF{\Offset,\Ones}\tcc*[r]{\Ones is a vector of ones of the size and position as \PHOCs.}
   }
   }
 }
 \FinalCharacterSums is a matrix the same size as \CharacterSums$[n]$\;
 \ForEach{\Char}{
   \ForEach{horizontal position along the word $p$}{
     \Contrib $:= 0$\;
     \ForEach{$n$ in \{$1,2,3$\}}{
       \If{\CharacterCounts$[n][$\Char$][p]!=0$}{
         \Contrib $:=$ \Contrib$+1$\;
         \FinalCharacterSums$[$\Char$][p] :=$ \FinalCharacterSums$[$\Char$][p] +$\CharacterSums$[n][$\Char$][p]/$\CharacterCounts$[n][$\Char$][p]$\;
       }
     }
     \eIf{\Contrib $==0$}{
       \FinalCharacterSums$[$\Char$][p] :=$ \MinValue\;
     }{
       \FinalCharacterSums$[$\Char$][p] :=$ \FinalCharacterSums$[$\Char$][p] / $\Contrib\;
     }
   } 
 }
 \ForEach{horizontal position along the word $p$}{
   \FinalCharacterSums$[*][p] :=$ \SoftMax{\FinalCharacterSums$[*][p]$}\tcc*[r]{Soft-max occurs across characters}
 }
 return \FinalCharacterSums
