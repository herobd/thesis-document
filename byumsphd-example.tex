...
\cite{sudholt2017} uses a cosine similarity to compare PHOC vectors (both string and network generated) and we follow the same procedure.
...
\begin{tabular}{| l | c  c | c c | c c |}
  ...
  Our network adapted PHOC  &  0.X & 0.X  &  0.888 & 0.912  &  0.X & 0.X
...
It can be seen that we underperform compared to \cite{sudholt2017}. This is to be expected as we have reduced the number of parameters in the  	CNN model.
...
\begin{tabular}{| l | c c | c c |}
  ...
  \hline
  %with cross-entropy
  Unigram &  0.608 &  0.338  & 0.X &  0.X \\
  Bigram  &  0.641 &  0.429  & 0.X &  0.X \\
  Trigram &  0.560 &  0.449  & 0.X &  0.X \\
  \hline  
  %mask-adapted
  Unigram &  0.654 &  0.X  & 0.X &  0.X \\ %cosine
  Bigram  &  0.679 &  0.X  & 0.X &  0.X \\ %drop 3,5
  Trigram &  0.612 &  0.X  & 0.X &  0.X \\ %drop 2,4,5
  \hline  
  %net-adapted + consine sim   old ww
  Unigram &  0.678 &  0.453  & 0.X &  0.X \\ 
  Bigram  &  0.643 &  0.535  & 0.X &  0.X \\
  Trigram &  0.618 &  0.533  & 0.X &  0.X \\
  \hline 
  %net-adapted + cross ent   old ww
  Unigram &  0.691 &  0.318  & 0.X &  0.X \\ 
  Bigram  &  0.665 &  0.408  & 0.X &  0.X \\
  Trigram &  0.640 &  0.431  & 0.X &  0.X \\
  \hline 
  %net-adapted + masked cosine sim   old ww
  Unigram &  0.499 &  0.296  & 0.X &  0.X \\ 
  Bigram  &  0.571 &  0.435  & 0.X &  0.X \\
  Trigram &  0.586 &  0.497  & 0.X &  0.X \\
  \hline 
  %net-adapted + masked cross ent   old ww
  Unigram &  0.691 &  0.318  & 0.497 &  0.203 \\ 
  Bigram  &  0.665 &  0.408  & 0.402 &  0.179 \\
  Trigram &  0.659 &  0.438  & 0.363 &  0.170 \\
  \hline 
\end{tabular}
...
%\begin{figure}
    ...
    \caption{This shows the QbS MAP for 18 n-grams on the Bentham dataset for varying sliding window sizes (in pixels). Figure \ref{fig:exampleWindows} shows examples of what these windows might look like.
    }
...
!!!!!THE NEW STUFF
We note that for QbS that the relative performance of unigrams, bigrams, and trigrams is reversed. This occurs becuase of an error where windows containing only part of the desired n-gram (e.g.~a single character of a bigram) are scored highly. For example...
The problem becomes most agrevated when tryign to spotting n-grams with double letters. As these errors do not occur at all in unigrams, it's MAP is higher. It can occur more freqently in the trigram case than bigram.
