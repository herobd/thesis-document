...
Creation figure:\\
a. "aligned forms"
b. contrast enhanced average
c. lines on contrast enhanced average
d. lines of form, arrows showing 50 pix scan %f. best point, 7x7 window scan (colored box)
e. histograms for borders (Downloads/makehist.py)
f. snapped image
g. word segmentation

\begin{figure}
    \centering
    \includegraphics[width=.75\textwidth]{makenames}
    \caption{Process for extracting names from US 1930 census forms for Census Names data set. We begin with registerd forms (a). Then we average the images together (b). The average image is used to mark form boundaries manually (c).}
    \label{fig:makenames}
\end{figure}
