...
Pseudo code is shown in Algorithm 1.%\ref{regex}.
...
\SetKwData{Ret}{regularExp}\SetKwData{RetTmp}{regularExpTmp}\SetKwData{Possible}{possibleWords}\SetKwData{Scores}{possibleScores}\SetKwData{MinScore}{minScore}\SetKwData{MaxScore}{maxScore}\SetKwData{Thresh}{thresholdScore}\SetKwData{MinN}{minNumOfChars}\SetKwData{MaxN}{maxNumOfChars}
...
\SetKwInOut{Input}{input}\SetKwInOut{Output}{output}
 ...
 \If{no words in \Possible}{
    \ForEach{spotting that do overlap with others}{
    	  temporarily remove the spotting\;
    	  \RetTmp $:=$ \Algorithm{new spotting set}\;
    	  \Possible $:=$ \Possible $\bigcup$ matches of \RetTmp from lexicon\;
    	}
...
\If{number of words in \Possible $< 49$}{
  \Scores $:= []$\;
  \ForEach{word in \Possible}{
    \Scores $\leftarrow$ \WordSpot{word image, possible word}\tcc*[r]{insert score into \Scores}
  }
  \Thresh $:=$ \Otsu{\Scores}\;
  ...
  \If{number of words in \Possible $< 7$}{
    return \CreateBatch{\Possible,\Scores}\;
  }
