...
If this fails to yield any lexicon matches, we try to account for the third scenario by regenerating the regular expression in a ``loose'' mode, where each range of unknown characters is expanded by one to allow matches initally constrained by a bad alignment estimation.
...
The simplest method is to simply start with the spotting with the highest score until the user begins to classify enough as false that  we can assume we are at a good thresholding location to reject the remaining spottings. 
...
%TODO replace this image too, mention that it is a normal distribution curve
\begin{figure}
    \centering
...
If we select two distributions, we estimate an initial split between true and false scores using the Otsu threshold. We initally model the two distributions assuming all spottings are true or false based on this threshold.
